      ...
      <div className="border border-border bg-card rounded-sm shadow-sm overflow-hidden">
        ...
        <div className="p-8 space-y-8">
          <div>
            ...
            <div className="flex gap-px bg-border border border-border mt-4">
              {[25, 50, 75, 100].map((v) => (
                <button 
                  key={v} 
                  onClick={() => setPercentage([v])} 
                  className={`flex-1 py-2 text-[10px] font-bold tracking-widest transition-colors ${percentage[0] === v ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground hover:bg-muted"}`}
                >
                  {v === 100 ? "MAXIMUM" : `${v}%`}
                </button>
              ))}
